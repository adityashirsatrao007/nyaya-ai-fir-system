...
def get_ipc_patterns() -> List[Tuple[str, re.Pattern]]:
    ...
    patterns = [
        ...
        ("section", re.compile(
            r'\bSection[s]?\s*(\d{1,3}[A-Z]?(?:\s*/\s*\d{1,3}[A-Z]?)*)(?!\d)',
            re.IGNORECASE
        )),
        
        # "Sec. 302" or "Sec 302"
        ("sec", re.compile(
            r'\bSec\.?\s*(\d{1,3}[A-Z]?(?:\s*/\s*\d{1,3}[A-Z]?)*)(?!\d)',
            re.IGNORECASE
        )),
        
        # "u/s 302" or "U/S 302" (under section)
        ("under_section", re.compile(
            r'\bu/s\.?\s*(\d{1,3}[A-Z]?(?:\s*/\s*\d{1,3}[A-Z]?)*)(?!\d)',
            re.IGNORECASE
        )),
        
        # "IPC 302" or "IPC Section 302"
        ("ipc_direct", re.compile(
            r'\bIPC\s*(?:Section[s]?)?\s*(\d{1,3}[A-Z]?(?:\s*/\s*\d{1,3}[A-Z]?)*)(?!\d)',
            re.IGNORECASE
        )),
        
        # "302 IPC" or "302/307 IPC"
        ("section_ipc", re.compile(
            r'\b(\d{1,3}[A-Z]?(?:\s*/\s*\d{1,3}[A-Z]?)*)\s*(?:of\s+)?IPC\b',
            re.IGNORECASE
        )),
        
        # "धारा 302" (Hindi)
        ("dhara", re.compile(
            r'धारा\s*(\d{1,3}[A-Z]?(?:\s*/\s*\d{1,3}[A-Z]?)*)(?!\d)',
            re.IGNORECASE
        )),
        
        # "302, 307, 420" - multiple sections
        ("comma_separated", re.compile(
            r'(?:Section[s]?|धारा|u/s|IPC)\s*(\d{1,3}[A-Z]?(?:\s*,\s*\d{1,3}[A-Z]?)+)(?!\d)',
            re.IGNORECASE
        )),
        ...
        ("contextual", re.compile(
            r'(?:charged|accused|booked|registered|filed|offense|offence|punishable|under)\s+(?:under\s+)?(\d{1,3}[A-Z]?)\b',
            re.IGNORECASE
        )),
        
        # "341|313|273" or "302/307/34" - sequences of numbers separated by symbols or spaces (common in OCR noise)
        ("number_sequence", re.compile(
            r'\b(\d{2,3}[A-Z]?(?:[|/,\-_\s]\d{2,3}[A-Z]?)+)(?!\d)',
            re.IGNORECASE
        )),
    ]
...
def extract_section_numbers(text: str) -> Tuple[List[str], List[str]]:
    ...
    # Track which patterns matched each section
    section_patterns = {}
    ...
    for pattern_name, pattern in patterns:
        ...
        for match in matches:
            ...
            # Handle multiple sections (separated by /, ,, |, _, -, or space)
            if any(char in match for char in ['/', ',', '|', '_', '-', ' ']):
                parts = re.split(r'[/,|_\-\s]\s*', match)
                for part in parts:
                    part = part.strip().upper()
                    if part and re.match(r'^\d{1,3}[A-Z]?$', part, re.IGNORECASE):
                        if part not in section_patterns:
                            section_patterns[part] = set()
                        section_patterns[part].add(pattern_name)
            else:
                match = match.strip().upper()
                if match and re.match(r'^\d{1,3}[A-Z]?$', match, re.IGNORECASE):
                    if match not in section_patterns:
                        section_patterns[match] = set()
                    section_patterns[match].add(pattern_name)
                    
    # Filter sections:
    # A section is kept if:
    # 1. It has at least one strong pattern match, OR
    # 2. It is present in the local IPC database keys, OR
    # 3. It's a very common section that we want to keep.
    strong_patterns = {"section", "sec", "under_section", "ipc_direct", "section_ipc", "dhara", "comma_separated"}
    
    filtered_sections = []
    for section, matched_patterns in section_patterns.items():
        # Check if it has a strong match
        has_strong = any(p in strong_patterns for p in matched_patterns)
        
        # Check if it is in the database
        in_db = section in db
        
        # Normalize: if not in db, try matching without suffix (e.g. 354A -> 354)
        if not in_db:
            base_match = re.match(r'^(\d+)', section)
            if base_match:
                in_db = base_match.group(1) in db
                
        # Also let's check for common CrPC sections that we want to exclude to prevent false positives
        # CrPC 154, 155, 156, 173, 174 are CrPC, not IPC!
        is_crpc = section in {"154", "155", "156", "173", "174"}
        
        if (has_strong or in_db) and not is_crpc:
...
def extract_ipc_sections(text: str) -> List[Dict]:
    ...
    falls back to semantic similarity using SentenceTransformers to detect
    relevant sections based on the crime description.
    
    Args:
        text: Input text containing IPC references or crime description
    ...
    # 1. Process explicit regex matched sections
    for section in sections:
        info = get_section_info(section)
        matched_sections_set.add(section.upper())
